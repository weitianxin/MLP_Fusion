...
class GPT2MLP(nn.Module):
    def __init__(self, intermediate_size, config, adapter_config=None, P_count_vec = nn.Identity()):
        super().__init__()
        embed_dim = config.hidden_size
        # 这里的Conv1D(output, input) 和 nn.Conv1d， nn.Linear 的参数顺序是反的
        self.c_fc = Conv1D(intermediate_size, embed_dim)
        self.c_proj = Conv1D(embed_dim, intermediate_size)
        self.act = ACT2FN[config.activation_function]
        self.dropout = nn.Dropout(config.resid_pdrop)
        self.P_count_vec = P_count_vec
...
class GPT2Block(nn.Module):
    def __init__(self, config, adapter_config=None):
        ...
        self.mlp = GPT2MLP(self.inner_dim, config, adapter_config=adapter_config, P_count_vec = self.P_count_vec)
    ...
    def update_sketch(self):
        ...
        # if self.config.ffn_mode == "sketch":
            ...
            score = (torch.linalg.vector_norm(self.mlp.c_fc.weight, dim=0) * 
                torch.linalg.vector_norm(self.mlp.c_proj.weight, dim=1))

            idx = torch.topk(score, k)[1]
            sketch_idx = torch.ones(self.inner_dim, dtype=torch.bool)
            sketch_idx[idx] = False
            sketch = 1/math.sqrt(self.config.ffn_bn - k) * torch.randn(
                    self.inner_dim - k, self.config.ffn_bn - k)

            
            # self.mlp.c_fc (768 x 3072)
            self.mlp.c_fc.nf = self.config.ffn_bn
            weight_select = self.mlp.c_fc.weight.data[:, idx]
            weight_sketch = self.mlp.c_fc.weight.data[:, sketch_idx] @ sketch
            self.mlp.c_fc.weight = nn.Parameter(torch.cat((weight_select, 
                                                        weight_sketch), dim=1))

            bias_select = self.mlp.c_fc.bias.data[idx]
            bias_sketch = sketch.T @ self.mlp.c_fc.bias.data[sketch_idx]
            self.mlp.c_fc.bias = nn.Parameter(torch.cat((bias_select, 
                                                                bias_sketch)))

            # self.mlp.c_proj (3072 x 768)
            weight_select = self.mlp.c_proj.weight.data[idx]
            weight_sketch = sketch.T @ self.mlp.c_proj.weight.data[sketch_idx]
            self.mlp.c_proj.weight = nn.Parameter(torch.cat((weight_select, 
                                                        weight_sketch), dim=0))
            if self.config.ffn_mode == "ntk_cluster":
                self.mlp.P_count_vec = ScaleLayer(torch.ones(self.config.ffn_bn))

    def update_clusters(self):
        if self.config.ffn_mode == "cluster":
            w1 = self.mlp.c_fc.weight
            b1 = self.mlp.c_fc.bias
            w2 = self.mlp.c_proj.weight
            b1_reshape = torch.unsqueeze(b1,1)
            # pdb.set_trace()
            # print("b1 shape:", b1.size())
            # print("b1 shape:", b1_reshape.size())
            cat_w = torch.cat((w1.T,b1_reshape,w2),1).detach().numpy()
            ...
            # print("dimension of center: ", centers.size())
            ...
            # pdb.set_trace()
            # print("w1 shape:", w1_new.size())
            # print("w2 shape:", w2_new.size())
            # print("b1 shape:", b1_new.size())
            # print("C shape:", C.size())
            # print("C.T@C original shape: ", (C.T@C).size())

            #C^T@C method
            # C_sqr = torch.sqrt(C.T@C)
            ...
            # print(C_sqr[0:20, 0:20])
            # print("the matrixes are not equal:",(False in torch.eq(C_diagonal, C_sqr)))

            # self.mlp.c_fc (768 x 3072)
            self.mlp.c_fc.nf = self.config.ffn_bn

            self.mlp.c_fc.weight = nn.Parameter((C_diagonal@w1_new).T)
            # self.mlp.c_fc.weight = nn.Parameter((w1_new.T@C_diagonal.T)
            self.mlp.c_fc.bias = nn.Parameter(C_diagonal@b1_new)
            self.mlp.c_proj.weight = nn.Parameter((w2_new.T@C_diagonal).T)
            
            #NOTE
            #remove C -- finished
            #tune skectch layer, lr, n_init, write a script to run multiple tasks
            #hint: nvidia-smi check availible gpu, tune cuda in .sh file
    ...
    def update_mmd(self):
        if self.config.ffn_mode == "mmd":
            ...
            w2 = self.mlp.c_proj.weight
            b1_reshape = torch.unsqueeze(b1,1)
            b2 = self.mlp.c_proj.bias
            cat_w = torch.cat((w1.T,b1_reshape,w2),1).detach().numpy()
            cluster_num = self.config.ffn_bn
            # km = KMeans(n_clusters = cluster_num, n_init = 30, algorithm = "lloyd", random_state=123, n_jobs = 4).fit(cat_w)
            # initial_tensor = torch.Tensor(km.cluster_centers_)
            ...
            scale = math.sqrt(self.inner_dim/self.config.ffn_bn)
            ...
            # print(w1_new.shape, b1_new.shape, w2_new.shape)
            # self.mlp.c_fc (768 x 3072)
            self.mlp.c_fc.nf = self.config.ffn_bn

            self.mlp.c_fc.weight = nn.Parameter(scale*w1_new.T)
            self.mlp.c_fc.bias = nn.Parameter(scale*b1_new)
            self.mlp.c_proj.weight = nn.Parameter(scale*w2_new)
            self.mlp.c_proj.bias = nn.Parameter(scale*b2)

    def update_ntk_clusters(self):
        ...
        self.mlp.P_count_vec = ScaleLayer(torch.unique(C, return_counts = True)[1])
        # print(self.P_count_vec)

        # self.mlp.c_fc (768 x 3072)
        self.mlp.c_fc.nf = self.config.ffn_bn

        self.mlp.c_fc.weight = nn.Parameter(w1_new.T)
        self.mlp.c_fc.bias = nn.Parameter(b1_new)
        self.mlp.c_proj.weight = nn.Parameter(w2_new)
...
class MMD_model(nn.Module):
    def __init__(self, cat_w_approx):
        super().__init__()
        self.cat_w_approx = nn.Parameter(cat_w_approx)
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def update_clusters(self, update_idx=None):
        print("-------------update FFN clusters-------------")
        
        layers = self.transformer.h
        if update_idx is None: update_idx = range(len(layers))
        for i in update_idx:
            print("update layer {}".format(i))
            layers[i].update_clusters()

    def update_ntk_clusters(self, update_idx=None):
        print("-------------update FFN ntk clusters-------------")
        
        layers = self.transformer.h
        if update_idx is None: update_idx = range(len(layers))
        for i in update_idx:
            print("update layer {}".format(i))
            layers[i].update_ntk_clusters()
