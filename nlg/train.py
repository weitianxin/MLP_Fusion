...
from utils import (get_dataset, make_logdir, get_adapter_config, pruning_model_custom,
                    update_sketch_after_init, update_mmd_after_init, 
                    update_clusters_after_init, update_ntk_clusters_after_init)

sys.path.append("..")
from petl.options import TuneArguments
from petl.utils import convert_str_to_list
...
def train():
    ...
    parser.add_argument("--weight_decay", type=float, default=0.0, help="coefficient for weight_decay")
    parser.add_argument("--warmup_ratio", type=float, default=0.0, help="ratio for warmup steps")
    ...
    arg_parser = HfArgumentParser((AdapterTrainingArguments, TuneArguments))
    adapter_args, sketch_args = arg_parser.parse_json_file(args.config)
    ...
    for k, v in vars(sketch_args).items():
        if not hasattr(config, k):
            setattr(config, k, v)

    setattr(config, "pretrained_model_name_or_path", args.model_name)
    ...
    ori_model = copy.deepcopy(model).to(args.device)
    ##########################################################################
    # Manages which parameters to fix in the model

    # Modify model trainable parameters as per adapter settings
    if config.ffn_mode == 'sketch':
        with torch.no_grad():
            model = update_sketch_after_init(model, 
                                convert_str_to_list(sketch_args.sketch_layers))
    if config.ffn_mode == 'mmd':
        # with torch.no_grad():
        model = update_mmd_after_init(model, convert_str_to_list(sketch_args.sketch_layers))
    # cluster
    if config.ffn_mode == 'cluster':
        with torch.no_grad():
            print("input cluster number: ", sketch_args.ffn_bn)
            model = update_clusters_after_init(model, convert_str_to_list(sketch_args.sketch_layers))
    
    if config.ffn_mode == 'ntk_cluster':
        with torch.no_grad():
            print("input cluster number: ", sketch_args.ffn_bn)
            model = update_ntk_clusters_after_init(model, convert_str_to_list(sketch_args.sketch_layers))

    if config.ffn_mode == 'prune':
        mask = torch.load("pretrain_prun/mask.pt")
        pruning_model_custom(model, mask, convert_str_to_list(sketch_args.sketch_layers))
    ...
    optimizer = AdamW(model.parameters(), lr=args.lr, 
                            weight_decay=args.weight_decay, correct_bias=True)
    ...
    if args.distillation:
        ...
        params_to_update = []
        for ii in convert_str_to_list(sketch_args.sketch_layers):
            params_to_update.append({"params": model.transformer.h[ii].mlp.c_fc.parameters()})
            params_to_update.append({"params": model.transformer.h[ii].mlp.c_proj.parameters()})
        opt_distill = torch.optim.AdamW(params_to_update, lr=args.lr)
        ...
        for epoch in tqdm(range(1)):
            for batch in train_dataloader:
                ...
                with torch.no_grad():
                    ori_hidden = ori_model(input_ids, token_type_ids=token_type_ids, labels=lm_labels, label_smooth=args.label_smooth, task=args.task, output_hidden_states=True)["hidden_states"]
                opt_distill.zero_grad()
                loss = model.forward_distill(input_ids, token_type_ids=token_type_ids, labels=lm_labels, label_smooth=args.label_smooth, task=args.task, output_hidden_states=True, ori_hidden=ori_hidden)
                loss.backward()
                opt_distill.step()
    ...
    warmup_duration = int(args.n_epochs * len(train_loader) * args.warmup_ratio)
