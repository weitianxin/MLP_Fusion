...
from utils import get_dataset_personalities, get_dataset, get_adapter_config, pruning_model_custom, \
                                                    update_sketch_after_init, update_mmd_after_init, update_clusters_after_init, update_ntk_clusters_after_init
import numpy as np
...
def run():
    ...
    model = GPT2LMHeadModel(config, adapter_config)
    if config.ffn_mode == 'sketch' or config.ffn_mode == 'mmd' or config.ffn_mode == 'cluster' or config.ffn_mode == 'ntk_cluster':
        with torch.no_grad():
            model = update_sketch_after_init(model, 
                                convert_str_to_list("4,5,6,7,8,9,10,11"))
    # if config.ffn_mode == 'mmd':
    #     # with torch.no_grad():
    #     model = update_mmd_after_init(model, convert_str_to_list("4,5,6,7,8,9,10,11"))
    # # cluster
    # if config.ffn_mode == 'cluster':
    #     with torch.no_grad():
    #         model = update_clusters_after_init(model, convert_str_to_list("4,5,6,7,8,9,10,11"))
    
    # if config.ffn_mode == 'ntk_cluster':
    #     with torch.no_grad():
    #         model = update_ntk_clusters_after_init(model, convert_str_to_list("4,5,6,7,8,9,10,11"))
    
    if config.ffn_mode == 'prune':
        mask = torch.load("pretrain_prun/mask.pt")
        pruning_model_custom(model, mask, convert_str_to_list("4,5,6,7,8,9,10,11"))

    # print(model.transformer.h[4].mlp.c_fc.weight.shape);exit()
