...
def update_sketch_after_init(model, layer_idx=None):
    model.update_sketch(layer_idx)
...
def update_mmd_after_init(model, layer_idx=None):
    model.update_mmd(layer_idx)
...
def update_clusters_after_init(model, layer_idx=None):
    model.update_clusters(layer_idx)
...
def update_ntk_clusters_after_init(model, layer_idx=None):
    model.update_ntk_clusters(layer_idx)
...
def pruning_model_custom(model, mask_dict, sketch_layers_list):
    ...
    for ii in sketch_layers_list:
        parameters_to_prune.append(model.transformer.h[ii].mlp.c_fc)
        mask_list.append(mask_dict['transformer.h.'+str(ii)+'.mlp.c_fc.weight_mask'])
        parameters_to_prune.append(model.transformer.h[ii].mlp.c_proj)
        mask_list.append(mask_dict['transformer.h.'+str(ii)+'.mlp.c_proj.weight_mask'])
