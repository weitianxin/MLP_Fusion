...
def see_weight_rate(model):
    ...
    for ii in range(12):
        ...
        sum_list = sum_list+float(model.roberta.encoder.layer[ii].intermediate.dense.weight.nelement())
        zero_sum = zero_sum+float(torch.sum(model.roberta.encoder.layer[ii].intermediate.dense.weight == 0))

        sum_list = sum_list+float(model.roberta.encoder.layer[ii].output.dense.weight.nelement())
        zero_sum = zero_sum+float(torch.sum(model.roberta.encoder.layer[ii].output.dense.weight == 0))
