...
import pickle
from petl.petl_factory import Adapter_Layer, softmax_gating, Linear, ScaleLayer
...
class Linear_svd(nn.Module):
    ...
    def update_svd(self):
      with torch.no_grad():
        ...
        U, S, Vh = torch.linalg.svd(self.weight.data)
        U_aprox, S_aprox, Vh_aprox = U[:, :self.k].clone().detach(), S[:self.k].clone().detach(), Vh[:self.k, :].clone().detach()
        
        del U, S, Vh
        
        self.weight.data = U_aprox@torch.diag(S_aprox)
        self.w1 = Parameter(Vh_aprox)
        self.svd = True
        
        del U_aprox, S_aprox, Vh_aprox
    ...
    def forward(self, input):
        if not self.svd:
            ...
            out_inter = F.linear(input, self.w1)
            out = F.linear(out_inter, self.weight, self.bias)
        return out

class Linear_sketch(nn.Module):
    r"""Applies a linear transformation to the incoming data: :math:`y = xA^T + b`
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True,
                 device=None, dtype=None, is_output_layer=False):
        factory_kwargs = {'device': device, 'dtype': dtype}
        super(Linear_sketch, self).__init__()
    ...
    def update_sketch(self, idx, sketch):
        current_device = self.weight.device
        
        
        sketch = sketch.to(current_device)
      
        sketch_idx = torch.ones(sketch.shape[0]+idx.shape[0], dtype=torch.bool)
        sketch_idx[idx] = False

        if self.is_output_layer:
            weight_select = self.weight.data[:, idx]
            weight_sketch = self.weight.data[:, sketch_idx] @ sketch
            self.weight = Parameter(torch.cat((weight_select, weight_sketch), dim=1))
        else:
            weight_select = self.weight.data[idx]
            weight_sketch = sketch.T @ self.weight.data[sketch_idx]
            self.weight = Parameter(torch.cat((weight_select, weight_sketch), dim=0))

            if self.bias:
              bias_select = self.bias.data[idx]
              bias_sketch = sketch.T @ self.bias.data[sketch_idx]
              self.bias = Parameter(torch.cat((bias_select, bias_sketch)))

    def forward(self, input):
        return F.linear(input, self.weight, self.bias)


def structure_prune(linear_layer, num_neurons_to_prune, feature='row'):
    # Calculate the L1-norm of each neuron's weights
    if feature == 'row':
      norms = torch.norm(linear_layer.weight.data, p=1, dim=1)
    elif feature == 'column':
      norms = torch.norm(linear_layer.weight.data, p=1, dim=0)

    # Determine the neurons to prune
    neuron_indices = norms.argsort()[:num_neurons_to_prune]

    # Create a mask for weights
    weight_mask = torch.ones_like(linear_layer.weight.data, dtype=torch.bool)

    if feature == 'row':
      weight_mask[neuron_indices] = False
      linear_layer.weight.data = linear_layer.weight.data[weight_mask].view(
          linear_layer.weight.size(0) - num_neurons_to_prune, linear_layer.weight.size(1)
      )
    elif feature == 'column':
      weight_mask[:, neuron_indices] = False
      linear_layer.weight.data = linear_layer.weight.data[weight_mask].view(
        linear_layer.weight.size(0), -1
      )
    
    
    
    if linear_layer.bias is not None:
        bias_mask = torch.ones_like(linear_layer.bias.data, dtype=torch.bool)
        bias_mask[neuron_indices] = False
        linear_layer.bias.data = linear_layer.bias.data[bias_mask]
        

class MMD_model(nn.Module):
    def __init__(self, cat_w_approx):
        super().__init__()
        self.cat_w_approx = Parameter(cat_w_approx, requires_grad=True)

    def forward(self, w_cat):
        loss = self.mmd_loss(w_cat, self.cat_w_approx)
        loss = Parameter(loss, requires_grad = True)
        return loss

    def imq_kernel(self, source, target, eps = 1e-7, z_var = 2):
        total = torch.cat([source, target], dim=0)
        z_dim = total.size(-1)
        C = 2 * z_dim * z_var
        kernel = C / (eps + C + torch.cdist(total, total, p=2))

        # Exclude diagonal elements
        result = kernel - kernel.diag()
...
class SwitchTransformersDenseActDense(nn.Module):
    def __init__(self, config: SwitchTransformersConfig):
        super().__init__()
        if config.ffn_mode == 'svd':
          self.wi = Linear_svd(config.d_model, config.d_ff, bias=False)
          self.wo = Linear_svd(config.d_ff, config.d_model, bias=False)
        elif config.ffn_mode == 'sketch':
          self.wi = Linear_sketch(config.d_model, config.d_ff, bias=False)
          self.wo = Linear_sketch(config.d_ff, config.d_model, bias=False, is_output_layer=True)      
        else:
        ...
        self.load_clustering = True
        self.layer_idx = 0
        self.is_decoder = False
        self.expert_idx = 0
        
        self.P_count_vec = nn.Identity()
        
        # print(self.P_count_vec.device)
    ...
    def update_cluster(self):
        ...
        de = 'decoder' if self.is_decoder else 'encoder'
        ...
        #directly calculate the diagonal matrix to replace C^T@C
        C_diagonal = torch.zeros(cluster_num, cluster_num)
        count = torch.unique(C, return_counts = True)[1]
        for i in range(cluster_num):
            C_diagonal[i][i] = count[i]
        C_diagonal = torch.sqrt(C_diagonal)


        self.wi.weight.data = (C_diagonal@w1_new).to(current_device)
        self.wo.weight.data = (w2_new.T@C_diagonal).to(current_device)
    ...
    def update_mmd(self):
        ...
        perm = torch.randperm(cat_w.shape[0])
        idx = perm[:cluster_num]
        initial_tensor = torch.Tensor(cat_w[idx])
        MMD = MMD_model(initial_tensor).to(current_device)#.cuda()
        cat_w_ori = torch.Tensor(cat_w).to(current_device)#.cuda()
        optimizer = torch.optim.AdamW(MMD.parameters(), lr=1e-2)
        for i in range(100):
            optimizer.zero_grad()
            loss = MMD(cat_w_ori)
            # print(loss)
            loss.backward()
            optimizer.step()
        cat_w_approx = MMD.cat_w_approx
        scale = math.sqrt(3072/768)
        w1_new = cat_w_approx[:, :768]
        w2_new = cat_w_approx[:, 768:]
        
        self.wi.weight.data = (scale*w1_new).to(current_device)
        self.wo.weight.data = (scale*w2_new.T).to(current_device)


    def update_sketch(self):
        # remove the top-k selection trick
        
        k = 256
        score = (torch.linalg.vector_norm(self.wi.weight, dim=1) * 
            torch.linalg.vector_norm(self.wo.weight, dim=0))

        idx = torch.topk(score, k)[1]

        sketch_matrix = 1/math.sqrt(768-k)*torch.randn(
                    3072-k, 768-k)

        self.wi.update_sketch(idx, sketch_matrix)
        self.wo.update_sketch(idx, sketch_matrix)

    def update_svd(self):
        self.wi.update_svd()
        self.wo.update_svd()

    def update_ntk_cluster(self):
        ...
        self.P_count_vec = ScaleLayer(torch.unique(C, return_counts = True)[1], device = current_device)
    ...
    def update_structure_pruning(self):
        k = 2304
        
          
        structure_prune(self.wi,k,"row")
        structure_prune(self.wo,k,"column")
              


    def update_unstructured_pruning(self):     
        current_device = self.wi.weight.device
      
        de = 'decoder' if self.is_decoder else 'encoder'
        
        mask = torch.load(f'./ntk_mask/mask_{self.layer_idx}_{self.expert_idx}_{de}_wi.pt', map_location=current_device)
        prune.custom_from_mask(self.wi, name='weight', mask=mask)
        
        del mask
        torch.cuda.empty_cache()
        
        mask = torch.load(f'./ntk_mask/mask_{self.layer_idx}_{self.expert_idx}_{de}_wo.pt', map_location=current_device)
        prune.custom_from_mask(self.wo, name='weight', mask=mask)
        
        del mask
        
        prune.remove(self.wi, 'weight')
        prune.remove(self.wo, 'weight')
    ...
    def forward(self, hidden_states):
        ...
        # hidden_states = self.P_count_vec(hidden_states)
...
class SwitchTransformersSparseMLP(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # print(router_mask,hidden_states.shape)
        ...
        for idx, expert in enumerate(self.experts.values()):
            ...
            # print(token_indices,hidden_states[token_indices].shape,expert(hidden_states[token_indices]).shape)
...
class SwitchTransformersPreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["SwitchTransformersBlock","ResmoeSwitchTransformersBlock"]
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, SwitchClassificationHead):
...
class SwitchTransformersStack(SwitchTransformersPreTrainedModel):
    def __init__(self, config, embed_tokens=None):
        ...
        for idx, i in enumerate(self.block):
          if self.is_decoder:
            if idx % 2 !=0:
              experts = i.layer[2].mlp.experts
              expert_indices = experts.keys()
              for eidx in expert_indices:
                experts[eidx].layer_idx = idx
                experts[eidx].expert_idx = eidx
                experts[eidx].is_decoder = True
            # print(i.layer[2].mlp)
          else:
            if idx % 2 !=0:
              experts = i.layer[1].mlp.experts
              expert_indices = experts.keys()
              for eidx in expert_indices:
                experts[eidx].layer_idx = idx
                experts[eidx].expert_idx = eidx
...
class SwitchClassificationHead(nn.Module):
...
# Copied from transformers.models.t5.modeling_t5.T5ForSequenceClassification with T5->SwitchTransformers     
class SwitchForSequenceClassification(SwitchTransformersPreTrainedModel):
    ...
    def __init__(self, config: SwitchTransformersConfig):
        ...
        self.classification_head = SwitchClassificationHead(config)
    ...
    def forward_distill(
            ...
            output_attentions: Optional[bool] = None,
            output_hidden_states: Optional[bool] = None,
            output_router_logits=None, # Set to True when needs to output router logits.
            return_dict: Optional[bool] = None,
            ori_hidden = None
        ):
            r"""
            ...
            for i in range(1, len(hidden_states)):
                loss += torch.mean(torch.sum(torch.square(hidden_states[i]-ori_hidden[i]), dim=-1))/(len(hidden_states)-1)
            return loss

        
        
    def update_sketch(self, update_idx=None):
        print("-------------update FFN sketch-------------")
        
        layers = self.switch_transformers.encoder.block
        if update_idx is None: update_idx = range(len(layers))
        for i in update_idx:
            print("updating layer", i)
            experts = layers[i].layer[1].mlp.experts
            expert_indices = experts.keys()
            for idx in expert_indices:
              experts[idx].update_sketch()
              
        layers = self.switch_transformers.decoder.block
        if update_idx is None: update_idx = range(len(layers))
        for i in update_idx:
            print("updating layer", i)
            experts = layers[i].layer[2].mlp.experts
            expert_indices = experts.keys()
            for idx in expert_indices:
              experts[idx].update_sketch()
        print("-------------update finish-------------")
    ...
    def update_clusters(self, update_idx=None):
        print("-------------update FFN ntk_cluster-------------")
    ...
    def update_ntk_clusters(self, update_idx=None):
        print("-------------update FFN ntk_cluster-------------")
        ...
        for i in update_idx:
            ...
            expert_indices = experts.keys()
            for idx in expert_indices:
              experts[idx].update_ntk_cluster()
    ...
    def update_mmd(self, update_idx=None):
        print("-------------update FFN mmd-------------")
        layers = self.switch_transformers.encoder.block
        if update_idx is None: update_idx = range(len(layers))
        for i in update_idx:
            print("updating layer", i)
    ...
    def update_svd(self, update_idx=None):
        print("-------------update FFN svd-------------")
        layers = self.switch_transformers.encoder.block
        if update_idx is None: update_idx = range(len(layers))
        for i in update_idx:
            print("updating layer", i)
    ...
    def update_structure_pruning(self, update_idx=None):
        print("-------------update FFN structure pruning-------------")
        layers = self.switch_transformers.encoder.block
        if update_idx is None: update_idx = range(len(layers))
        for i in update_idx:
            print("updating layer", i)
            ...
            expert_indices = experts.keys()
            for idx in expert_indices:
              experts[idx].update_structure_pruning()
    ...
    def update_unstructured_pruning(self, update_idx=None):    
        print("-------------update FFN structure pruning-------------")
        layers = self.switch_transformers.encoder.block
        if update_idx is None: update_idx = range(len(layers))
        for i in update_idx:
            ...
            expert_indices = experts.keys()
            for idx in expert_indices:
              experts[idx].update_unstructured_pruning()
