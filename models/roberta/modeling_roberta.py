    ...
    *docstr, tokenizer_class=None, checkpoint=None, output_type=None, config_class=None, mask=None, model_cls=None
    ...
    def docstring_decorator(fn):
        ...
        if "SequenceClassification" in model_class:
            code_sample = PT_SEQUENCE_CLASSIFICATION_SAMPLE
...
class Linear_svd(nn.Module):
    ...
    def update_svd(self):
        ...
        U, S, Vh = torch.linalg.svd(ori_weight)
        U_aprox, S_aprox, Vh_aprox = U[:, :self.t], S[:self.t], Vh[:self.t, :]
        self.weight, self.w1 = Parameter(U_aprox@torch.diag(S_aprox)), Parameter(Vh_aprox)
        self.svd=True
...
class RobertaIntermediate(nn.Module):
    def __init__(self, config):
        ...
        if config.ffn_mode == 'lora':
            ...
            self.dense = Linear_sketch(config.hidden_size, config.intermediate_size)
        elif config.ffn_mode == 'svd':
            self.dense = Linear_svd(config.hidden_size, config.intermediate_size)
        else:
...
class RobertaOutput(nn.Module):
    def __init__(self, config):
        ...
        if config.ffn_mode == 'lora':
            ...
            self.dense = Linear_svd(config.intermediate_size, config.hidden_size)
        elif config.ffn_mode == 'sketch':
            self.dense = Linear_sketch(config.intermediate_size, config.hidden_size, is_output_layer=True)
        else:
...
class RobertaLayer(nn.Module):
    ...
    def update_sketch_trick(self):

        if self.config.ffn_mode == "sketch":
            k = self.config.mid_dim
            score = (torch.linalg.vector_norm(self.intermediate.dense.weight, dim=1) * 
                torch.linalg.vector_norm(self.output.dense.weight, dim=0))

            idx = torch.topk(score, k)[1]

            # sketch_matrix = 1/math.sqrt(config.ffn_bn)*torch.randn(config.intermediate_size, config.ffn_bn).cuda()
            sketch_matrix = 1/math.sqrt(self.config.ffn_bn-k)*torch.randn(
                        self.config.intermediate_size-k, self.config.ffn_bn-k)
            # self.sketch_matrix = torch.eye(config.intermediate_size).cuda()
            # self.sketch_matrix

            self.intermediate.dense.update_sketch(idx, sketch_matrix)
            self.output.dense.update_sketch(idx, sketch_matrix)

    def update_cluster(self):
        if self.config.ffn_mode == "cluster":
            w1 = self.intermediate.dense.weight
            b1 = self.intermediate.dense.bias
            w2 = self.output.dense.weight
            b1_reshape = torch.unsqueeze(b1,1)
            # pdb.set_trace()
            # print("b1 shape:", b1.size())
            # print("b1 shape:", b1_reshape.size())
            cat_w = torch.cat((w1,b1_reshape,w2.T),1).detach().numpy()
            # print("cat_w shape:", cat_w.size())
            cluster_num = self.config.ffn_bn
            # print("cluster num is: ", cluster_num)

            if self.config.load_clustering:
                # print(sys.path)
                with open("clustering/{}_{}_{}_{}.pkl".format(
                        self.config.pretrained_model_name_or_path, 
                        self.config.ffn_bn, self.config.seed, self.layer_idx), 
                            'rb') as f:
                    km = pickle.load(f)
                # sys.exit()
            else:
            ...
            w1_new = centers[:, :self.config.hidden_size]
            b1_new = centers[:, self.config.hidden_size]
            w2_new = centers[:, self.config.hidden_size+1:]
            ...
            self.intermediate.dense.weight = Parameter(C_diagonal@w1_new)
            self.intermediate.dense.bias = Parameter(C_diagonal@b1_new)
            self.output.dense.weight = Parameter(w2_new.T@C_diagonal)



    def update_mmd(self):
        if self.config.ffn_mode == "mmd":
            w1 = self.intermediate.dense.weight
            b1 = self.intermediate.dense.bias
            w2 = self.output.dense.weight
            b1_reshape = torch.unsqueeze(b1,1)
            b2 = self.output.dense.bias
            cat_w = torch.cat((w1,b1_reshape,w2.T),1).detach().numpy()
            cluster_num = self.config.ffn_bn
            ...
            MMD = MMD_model(initial_tensor).cuda()
            cat_w_ori = torch.Tensor(cat_w).cuda()
            optimizer = torch.optim.AdamW(MMD.parameters(), lr=1e-2)
            for i in range(int(self.config.n_steps)):
                optimizer.zero_grad()
                loss = MMD(cat_w_ori)
                loss.backward()
                optimizer.step()
            cat_w_approx = MMD.cat_w_approx
            scale = math.sqrt(self.config.intermediate_size/self.config.ffn_bn)
            w1_new = cat_w_approx[:, :self.config.hidden_size]
            b1_new = cat_w_approx[:, self.config.hidden_size]
            w2_new = cat_w_approx[:, self.config.hidden_size+1:]

            self.intermediate.dense.weight = Parameter(scale*w1_new)
            self.intermediate.dense.bias = Parameter(scale*b1_new)
            self.output.dense.weight = Parameter(scale*w2_new.T)
            self.output.dense.bias = Parameter(scale*b2)
    ...
    def update_svd(self):
        self.intermediate.dense.update_svd()
        self.output.dense.update_svd()
    ...
    def update_ntk_cluster(self):
        ...
        self.intermediate.dense.weight = Parameter(w1_new)
        self.intermediate.dense.bias = Parameter(b1_new)
        self.output.dense.weight = Parameter(w2_new.T)
    ...
    def feed_forward_chunk(self, attention_output):
        ...
        # intermediate_output = self.intermediate(self.output.LayerNorm(attention_output))
        intermediate_output = self.intermediate(attention_output)
        intermediate_output = self.P_count_vec(intermediate_output)
...
class MMD_model(nn.Module):
    def __init__(self, cat_w_approx):
        super().__init__()
        self.cat_w_approx = Parameter(cat_w_approx)
...
class RobertaModel(RobertaPreTrainedModel):
    ...
    def set_bias_tuning_params(self):
        # added by Chunting: stranded
        if self.config.tuning_option == 'add_to_input':
            self.embeddings.learnable_bias_embeddings = nn.Embedding(self.config.max_position_embeddings, self.config.hidden_size)
        elif self.config.tuning_option == 'add_to_att_output' or self.option == 'add_to_input_every_layer':
        ...
        elif self.config.tuning_option == 'prefix_tuning':
            pass
            #self.encoder.learnable_bias_embeddings = nn.ModuleList([nn.Embedding()])
...
class RobertaForSequenceClassification(RobertaPreTrainedModel):
    ...
    def update_sketch(self, update_idx=None):
        ...
        for i in update_idx:
            ...
            layers[i].update_sketch()
        print("-------------update finish-------------")

    def update_clusters(self, update_idx=None):
        print("-------------update FFN cluster-------------")
        
        layers = self.roberta.encoder.layer
        if update_idx is None: update_idx = range(len(layers))
        for i in update_idx:
            print("updating layer", i)
            layers[i].update_cluster()
    ...
    def reinit(self, reinit_idx=[]):
        ...
        def weight_reset(m):
            ...
            if callable(reset_parameters):
                ...
                print("no function to re-init")
                sys.exit()

        print("-------------re-init layer paras-------------")
        
        layers = self.roberta.encoder.layer
        for i in reinit_idx:
            layers[i].intermediate.dense.apply(weight_reset)
            layers[i].output.dense.apply(weight_reset)
            print("reinit layer", i)
    ...
    def forward_distill(
        ...
        loss = 0
        for i in range(1, 13):
            loss += torch.mean(torch.sum(torch.square(hidden_states[i]-ori_hidden[i]), dim=-1))/12
        return loss
